...
# ───────── helpers ─────────
def team_keys():
    teams = _get("https://api.sportsdata.io/v3/nba/scores/json/Teams", "Teams")
    m = {}
    for t in teams:
        full = f"{t['City']} {t['Name']}"     # "Miami Heat"
        m[full.lower()] = t["Key"]
        m[t["Name"].lower()] = t["Key"]       # "Heat"
        m[t["City"].lower()] = t["Key"]       # "Miami"
    return m
...
def points(game):
    ...
    if a or h:
        return a, h
    a_tot = h_tot = 0
    for q in game.get("Quarters", []):
        a_tot += q.get("AwayScore", 0)
        h_tot += q.get("HomeScore", 0)
    return a_tot, h_tot

def season_year(d: datetime.date):
    # NBA season is labelled by the year in which it starts
...
def locate(date_str, k1, k2):
    ...
    for ds in (base, base + timedelta(days=1)):
        url = f"https://api.sportsdata.io/v3/nba/scores/json/GamesByDateFinal/{ds}"
    ...
    for tag in ("POST", ""):
        ...
        url = f"https://api.sportsdata.io/v3/nba/scores/json/Games/{season}"
