...
# ───────── helpers ─────────
def team_keys():
    teams = _get("https://api.sportsdata.io/v3/cbb/scores/json/Teams", "Teams")
    m = {}
    for t in teams:
        school = t['School']                      # "Duke"
        m[school.lower()] = t["Key"]
        # Add common team name variations
        if " " in school:
            parts = school.split()
            m[parts[0].lower()] = t["Key"]        # "Duke" from "Duke Blue Devils"
            m[" ".join(parts[1:]).lower()] = t["Key"]  # "Blue Devils" from "Duke Blue Devils"
...
def points(game):
    ...
    if a or h:
        return a, h
    # CBB uses Periods (halves) instead of Quarters
    a_tot = h_tot = 0
...
def season_year(d: datetime.date):
    # CBB season is labeled by the year in which it starts (fall)
...
def locate(date_str, k1, k2):
    ...
    for ds in (base, base + timedelta(days=1)):
        url = f"https://api.sportsdata.io/v3/cbb/scores/json/GamesByDateFinal/{ds}"
    ...
    for tag in ("POST", ""):
        ...
        url = f"https://api.sportsdata.io/v3/cbb/scores/json/Games/{season}"
...
if __name__ == "__main__":
    DATE   = "2025-03-15"
    TEAM1  = "Duke Blue Devils"   # p1
    TEAM2  = "North Carolina"     # p2
