...
def main():
    ...
    """
    target_datetime_str = "2025-06-17 04:00:00"
    target_timezone = "US/Eastern"
    symbol = "ETHUSDT"

    # Convert target datetime to UTC
    local = pytz.timezone(target_timezone)
    naive = datetime.strptime(target_datetime_str, "%Y-%m-%d %H:%M:%S")
    local_dt = local.localize(naive, is_dst=None)
    utc_dt = local_dt.astimezone(pytz.utc)

    # Get the price change percentage
    price_change_percent = get_eth_price_change(symbol, utc_dt)
