...
RESOLUTION_MAP = {
    "Trail Blazers": "p2",  # Trail Blazers win maps to p2
    "Jazz": "p1",  # Jazz win maps to p1
...
def fetch_game_data():
    ...
    """
    date = "2025-04-09"
    home_team = "POR"  # Portland Trail Blazers abbreviation
    away_team = "UTA"  # Utah Jazz abbreviation
...
def determine_resolution(game):
    ...
    logger.info(
        f"Game status: {status}, Score: Jazz {away_score} - Trail Blazers {home_score}"
    )

    if status == "Final":
        if home_score > away_score:
            logger.info("Trail Blazers won")
            return RESOLUTION_MAP["Trail Blazers"]
        else:
            logger.info("Jazz won")
            return RESOLUTION_MAP["Jazz"]
