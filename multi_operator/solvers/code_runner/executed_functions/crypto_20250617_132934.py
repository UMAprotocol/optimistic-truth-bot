...
def get_solana_price_data(date_str, hour=8, minute=0, timezone_str="US/Eastern"):
    """
    Fetches the 1-hour candle data for Solana (SOL/USDT) on Binance at a specific time on a given date.

    Args:
        date_str: Date in YYYY-MM-DD format
        hour: Hour in 24-hour format (default: 8)
...
def main():
    """
    Main function to handle the Solana price query for June 17, 2025, 8AM ET.
    """
    query_date = "2025-06-17"
    query_hour = 8  # 8 AM ET
    query_timezone = "US/Eastern"

    try:
        open_price, close_price = get_solana_price_data(query_date, query_hour, 0, query_timezone)
