...
RESOLUTION_MAP = {
    "STL": "p2",  # St. Louis Cardinals maps to p2
...
def fetch_game_data(date):
    ...
    try:
        ...
        # Find the specific game between St. Louis Cardinals and New York Mets
        for game in games:
            if (game['HomeTeam'] == "STL" and game['AwayTeam'] == "NYM") or (game['HomeTeam'] == "NYM" and game['AwayTeam'] == "STL"):
                logger.info(f"Found game between St. Louis Cardinals and New York Mets on {date}")
                return game

        logger.warning("No matching game found between St. Louis Cardinals and New York Mets on the specified date.")
        return None
