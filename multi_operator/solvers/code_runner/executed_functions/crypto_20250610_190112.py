...
def resolve_market():
    ...
    target_date = datetime(2025, 6, 10, 14, 0, 0, tzinfo=pytz.timezone('US/Eastern'))  # June 10, 2025, 2 PM ET
    start_time_utc = int(target_date.timestamp() * 1000)
    end_time_utc = start_time_utc + 3600000  # Plus one hour

    # Fetch the closing price at the start and end of the interval
    try:
        start_price = fetch_price_data("BTCUSDT", "1h", start_time_utc, start_time_utc + 60000)  # 1-minute interval
        end_price = fetch_price_data("BTCUSDT", "1h", end_time_utc, end_time_utc + 60000)  # 1-minute interval
