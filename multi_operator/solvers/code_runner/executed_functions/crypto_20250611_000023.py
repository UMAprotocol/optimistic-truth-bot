...
def resolve_market(symbol, target_datetime):
    ...
    Args:
        ...
        target_datetime (datetime): The datetime for which the price change is to be checked.
    
    Returns:
        str: 'p1' if the price went down, 'p2' if the price went up, 'p3' if unknown.
    """
    # Convert target datetime to UTC and get the start and end times in milliseconds
    ...
    start_price = fetch_price_data(symbol, "1h", start_time, start_time + 60000)  # 1 minute after start
    end_price = fetch_price_data(symbol, "1h", end_time, end_time + 60000)  # 1 minute after end
...
def main():
    # Example usage
    symbol = "BTCUSDT"
    target_datetime = datetime(2023, 6, 10, 19, 0, tzinfo=pytz.timezone("US/Eastern"))
    try:
