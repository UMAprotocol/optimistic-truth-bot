...
RESOLUTION_MAP = {
    ...
    "50-50": "p3",  # Canceled or unresolved
...
def fetch_game_data(date):
    ...
    try:
        ...
        # Find the specific game between Ottawa Senators and Toronto Maple Leafs
        for game in games:
            if (game['HomeTeam'] == "OTT" and game['AwayTeam'] == "TOR") or (game['HomeTeam'] == "TOR" and game['AwayTeam'] == "OTT"):
                logger.info(f"Found game: Ottawa Senators vs Toronto Maple Leafs")
                return game

        logger.warning("No matching game found for Ottawa Senators vs Toronto Maple Leafs.")
        return None
