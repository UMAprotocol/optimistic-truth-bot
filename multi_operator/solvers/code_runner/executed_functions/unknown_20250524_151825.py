...
if __name__ == "__main__":
    ...
    TERM = "Beast"
    URL = "https://api.sportsdata.io/v3/cbb/scores/json/GamesByDate/{date}"
    ...
    if datetime.now().date() < datetime.strptime(EVENT_DATE, "%Y-%m-%d").date():
        ...
        formatted_url = URL.format(date=EVENT_DATE)
        response = _get(formatted_url, "EventCheck")
        if response:
            # Assuming the response contains a list of games/events
            for event in response:
                if TERM in event.get("Summary", ""):
                    print("recommendation: p2")  # Term found
                    break
            else:
                print("recommendation: p1")  # Term not found
        else:
            print("recommendation: p1")  # No data or event not found