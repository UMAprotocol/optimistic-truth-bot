...
def check_price_dip_to_threshold(symbol, start_date, end_date, threshold_price):
    ...
    dipped_to_threshold = False

    while current_start_ts < end_ts:
        current_end_ts = min(current_start_ts + 86400000, end_ts)  # 1 day in milliseconds
        data = fetch_price_data(symbol, current_start_ts, current_end_ts)
        if data:
            for candle in data:
                low_price = float(candle[3])  # Low price is at index 3 in the candle data
                if low_price <= threshold_price:
                    dipped_to_threshold = True
                    break
        if dipped_to_threshold:
            break
        current_start_ts += 86400000  # Move to the next day

    return dipped_to_threshold
