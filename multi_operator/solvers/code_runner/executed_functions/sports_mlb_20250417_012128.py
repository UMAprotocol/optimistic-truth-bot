...
def fetch_game_data(date):
    ...
    try:
        ...
        games = response.json()
        for game in games:
            if game['HomeTeam'] == 'ARI' and game['AwayTeam'] == 'MIA':
                return game
            elif game['HomeTeam'] == 'MIA' and game['AwayTeam'] == 'ARI':
                return game
        return None
...
def determine_resolution(game):
    ...
    if status == "Final":
        ...
        if home_score > away_score:
            winner = "Diamondbacks" if home_team == "ARI" else "Marlins"
        else:
            winner = "Marlins" if away_team == "MIA" else "Diamondbacks"
