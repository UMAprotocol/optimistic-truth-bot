...
def main():
    ...
    close_price_start = fetch_price_data(symbol, interval, start_time, start_time + 60000)  # Start of the hour
    close_price_end = fetch_price_data(symbol, interval, end_time - 60000, end_time)  # End of the hour
    
    if close_price_start and close_price_end:
        # Calculate the percentage change
        close_price_start = float(close_price_start)
        close_price_end = float(close_price_end)
