...
RESOLUTION_MAP = {
    "Hawks": "p2",  # Hawks win maps to p2
    "Nets": "p1",  # Nets win maps to p1
...
def determine_resolution(game):
    ...
    logger.info(
        f"Game status: {status}, Score: ATL {home_score} - BKN {away_score}"
    )

    if status == "Final":
        if home_score > away_score:
            logger.info("Home team (Hawks) won, resolving as Hawks")
            return RESOLUTION_MAP["Hawks"]
        else:
            logger.info("Away team (Nets) won, resolving as Nets")
            return RESOLUTION_MAP["Nets"]
