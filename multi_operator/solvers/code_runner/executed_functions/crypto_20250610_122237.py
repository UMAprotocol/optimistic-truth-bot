...
def get_price_change(symbol, target_time):
    """
    Calculates the price change percentage for the specified symbol at the given target time.
    """
...
def main():
    ...
    """
    symbol = "BTCUSDT"
    target_time = datetime(2025, 6, 10, 0, 0, tzinfo=pytz.timezone("US/Eastern"))

    try:
        price_change = get_price_change(symbol, target_time)
        logger.info(f"Price change percentage: {price_change}%")
