...
def fetch_game_data(date):
    ...
    """
    logger.info(f"Fetching game data for MLB on {date}")
    ...
    try:
        ...
        # Find the specific game between New York Yankees and Tampa Bay Rays
        for game in games:
...
def determine_resolution(game):
    ...
    logger.info(f"Game status: {status}, {away_team} ({away_score}) vs {home_team} ({home_score})")
    ...
    if status == "Final":
        ...
        if winner == "NYY":
            return RESOLUTION_MAP["NYY"]
        elif winner == "TBR":
            return RESOLUTION_MAP["TBR"]
