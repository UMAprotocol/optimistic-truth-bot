...
def resolve_market(symbol, target_datetime):
    ...
    Returns:
        str: Market resolution ('p1' for down, 'p2' for up, 'p3' for unknown).
    """
    ...
    if start_price is None or end_price is None:
        return "p3"  # Unknown if any price fetch fails
...
def main():
    # Example usage: Resolve the market for BTC/USDT on June 14, 2025, 9 PM ET
    target_datetime = datetime(2025, 6, 14, 21, 0, 0, tzinfo=pytz.timezone("US/Eastern"))
    resolution = resolve_market("BTCUSDT", target_datetime)
