...
def resolve_market():
    ...
    target_date = datetime(2025, 6, 13, 15, 0)  # June 13, 2025, 3 PM ET
    # Convert to UTC and then to milliseconds
    target_date_utc = target_date - timedelta(hours=4)  # ET is UTC-4
    start_time_ms = int(target_date_utc.timestamp() * 1000)
    end_time_ms = start_time_ms + 3600000  # 1 hour later

    try:
        # Fetch closing prices at the start and end of the 1-hour interval
        start_price = fetch_price_data("BTCUSDT", "1h", start_time_ms, start_time_ms + 60000)
        end_price = fetch_price_data("BTCUSDT", "1h", end_time_ms - 60000, end_time_ms)
