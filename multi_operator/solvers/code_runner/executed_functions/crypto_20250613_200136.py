...
def resolve_market(symbol, target_datetime):
    ...
    start_time = int((utc_datetime - timedelta(minutes=60)).timestamp() * 1000)
    end_time = int(utc_datetime.timestamp() * 1000)
    
    try:
        start_price = fetch_price_data(symbol, "1h", start_time, start_time + 60000)
        end_price = fetch_price_data(symbol, "1h", end_time, end_time + 60000)
...
def main():
    # Example usage
    symbol = "ETHUSDT"
    target_datetime = datetime(2025, 6, 13, 15, 0, tzinfo=pytz.timezone("US/Eastern"))
