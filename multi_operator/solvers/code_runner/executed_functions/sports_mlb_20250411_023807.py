...
def fetch_game_data():
    ...
    """
    date = "2025-04-10"
    home_team = "CAR"  # NHL abbreviation for Carolina Hurricanes
    away_team = "WAS"  # NHL abbreviation for Washington Capitals
...
def determine_resolution(game):
    ...
    logger.info(
        f"Game status: {status}, Score: Hurricanes {home_score} - Capitals {away_score}"
    )

    if status == "Final":
        if home_score > away_score:
            logger.info("Hurricanes won")
            return RESOLUTION_MAP["Hurricanes"]
        elif away_score > home_score:
            logger.info("Capitals won")
