...
def check_player_performance(date, player_name, team_vs):
    ...
    if games:
        for game in games:
            if team_vs in (game['HomeTeam'], game['AwayTeam']):
                # Find the player stats in the game
                game_id = game['GameID']
    ...
    return "p1"  # Default to "No" if game is not found or player did not play

# Main execution
if __name__ == "__main__":
    result = check_player_performance(GAME_DATE, PLAYER_NAME, TEAM_VS)
