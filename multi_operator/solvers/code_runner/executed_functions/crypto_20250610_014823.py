...
def main():
    ...
    target_time_local = tz.localize(datetime.strptime(f"{target_date} {target_hour}:00:00", "%Y-%m-%d %H:%M:%S"))
    target_time_utc = target_time_local.astimezone(timezone.utc)
    start_time_ms = int(target_time_utc.timestamp() * 1000)

    # Fetch the candle data
    try:
        data = get_data("BTCUSDT", "1h", start_time_ms)
        resolution = analyze_candle_data(data)
