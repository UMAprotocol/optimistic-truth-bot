...
RESOLUTION_MAP = {
    "MIL": "p2",  # Milwaukee Brewers maps to p2
    "SF": "p1",  # San Francisco Giants maps to p1
...
def fetch_game_data(date):
    ...
    try:
        ...
        # Find the specific game between Milwaukee Brewers and San Francisco Giants
        for game in games:
            if (game['HomeTeam'] == "SF" and game['AwayTeam'] == "MIL") or (game['HomeTeam'] == "MIL" and game['AwayTeam'] == "SF"):
...
def determine_resolution(game):
    ...
    logger.info(f"Game status: {status}, {home_team} vs {away_team}, Score: {home_score}-{away_score}")
