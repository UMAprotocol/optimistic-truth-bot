...
def main():
    ...
    target_time_local = datetime.strptime(f"{target_date_str} {target_time_str}", "%Y-%m-%d %H:%M")
    target_time_local = tz.localize(target_time_local)
    ...
    try:
        ...
        logging.info(f"Close price for {symbol} at {target_time_utc} UTC: {close_price}")

        # Determine the resolution based on the close price
        if close_price >= 82000.01:
