...
def resolve_market():
    ...
    symbol = "ETHUSDT"
    interval = "1h"
    target_time = datetime(2025, 6, 14, 7, 0, 0, tzinfo=pytz.timezone("US/Eastern"))
    target_time_utc = target_time.astimezone(pytz.utc)
    start_time = int(target_time_utc.timestamp() * 1000)
    end_time = start_time + 3600000  # 1 hour later in milliseconds

    try:
        closing_price_start = fetch_price_data(symbol, interval, start_time, start_time + 60000)  # 1 minute range for the exact hour
        closing_price_end = fetch_price_data(symbol, interval, end_time - 60000, end_time)  # 1 minute range for the end of the hour
