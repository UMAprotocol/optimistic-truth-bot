...
def resolve_market():
    ...
    target_date = datetime(2025, 6, 6, 17, 0, 0, tzinfo=pytz.timezone('US/Eastern'))  # June 6, 2025, 5 PM ET
    ...
    try:
        ...
        logger.info(f"Closing price for BTC/USDT at {target_date.strftime('%Y-%m-%d %H:%M %Z')} is {closing_price}")
        
        # Compare with the opening price to determine the market resolution
        opening_price = get_binance_price("BTCUSDT", start_time_ms - 3600000)  # 1 hour earlier
