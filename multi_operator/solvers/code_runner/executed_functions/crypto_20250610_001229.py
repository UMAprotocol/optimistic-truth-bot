import requests
import os
from datetime import datetime, timedelta
import pytz
from dotenv import load_dotenv

# Load environment variables from .env file
load_dotenv()

# API keys are not required for this specific task as we are accessing public data from Hyperliquid
# However, if needed, they can be loaded like this:
# BINANCE_API_KEY = os.getenv("BINANCE_API_KEY")

def fetch_hyperliquid_data(start_time, end_time):
    """
    Fetches the 1-minute candle data for HYPE/USDC from Hyperliquid between specified start and end times.
    
    Args:
        start_time (int): Start time in milliseconds since epoch.
        end_time (int): End time in milliseconds since epoch.
    
    Returns:
        list: List of candle data where each candle is a list containing [open, high, low, close, volume].
    """
    url = "https://app.hyperliquid.xyz/trade/HYPE/USDC"
    params = {
        "interval": "1m",
        "startTime": start_time,
        "endTime": end_time
    }
    try:
        response = requests.get(url, params=params)
        response.raise_for_status()
        data = response.json()
        return data
    except requests.RequestException as e:
        print(f"Failed to fetch data from Hyperliquid: {e}")
        return None

def check_price_dip_to_twelve(data):
    """
    Checks if the price dipped to $12.000 or lower in any of the fetched candles.
    
    Args:
        data (list): List of candle data.
    
    Returns:
        bool: True if price dipped to $12.000 or lower, False otherwise.
    """
    for candle in data:
        if float(candle[2]) <= 12.000:  # candle[2] is the 'low' price of the candle
            return True
    return False

def main():
    # Define the time range for the query
    start_date = datetime(2025, 5, 7, 16, 0, 0, tzinfo=pytz.timezone('US/Eastern'))
    end_date = datetime(2025, 5, 31, 23, 59, 59, tzinfo=pytz.timezone('US/Eastern'))
    
    # Convert to UTC and then to milliseconds since epoch
    start_time_utc = start_date.astimezone(pytz.utc)
    end_time_utc = end_date.astimezone(pytz.utc)
    start_time_ms = int(start_time_utc.timestamp() * 1000)
    end_time_ms = int(end_time_utc.timestamp() * 1000)
    
    # Fetch the data
    data = fetch_hyperliquid_data(start_time_ms, end_time_ms)
    
    # Check if the price dipped to $12.000 or lower
    if data and check_price_dip_to_twelve(data):
        print("recommendation: p2")  # Yes, it dipped to $12 or lower
    else:
        print("recommendation: p1")  # No, it did not dip to $12 or lower

if __name__ == "__main__":
    main()