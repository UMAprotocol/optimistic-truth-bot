...
def resolve_market():
    ...
    """
    # Define the specific date and time for the market
    target_date = datetime(2025, 5, 30, 22, 0, 0, tzinfo=pytz.timezone("US/Eastern"))
    target_date_utc = target_date.astimezone(pytz.utc)
    start_time_ms = int(target_date_utc.timestamp() * 1000)
    
    try:
        # Get the closing price at the start of the 10 PM ET candle
        closing_price_start = get_binance_price("BTCUSDT", start_time_ms)
        # Get the closing price at the start of the 11 PM ET candle (1 hour later)
        closing_price_end = get_binance_price("BTCUSDT", start_time_ms + 3600000)
