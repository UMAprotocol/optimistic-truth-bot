...
def resolve_market():
    ...
    target_date = datetime(2025, 6, 4, 5, 0, 0, tzinfo=pytz.timezone("US/Eastern"))
    target_date_utc = target_date.astimezone(pytz.utc)
    start_time_ms = int(target_date_utc.timestamp() * 1000)

    try:
        # Get the closing price at the start of the 5 AM ET candle
        closing_price_start = get_binance_price("BTCUSDT", start_time_ms)
        # Get the closing price at the start of the 6 AM ET candle (1 hour later)
