...
def get_eth_price_change(date_str, hour=12, minute=0, timezone_str="US/Eastern"):
    """
    Fetches the percentage change for the ETH/USDT pair for the 1-hour candle starting at the specified time.
...
def main():
    ...
    query_date = "2025-06-16"  # Date of interest
    query_hour = 12  # Hour of interest (12 PM ET)
    
    logger.info(f"Processing query for ETH/USDT price change on {query_date} at {query_hour}:00 ET")
    
    try:
        price_change = get_eth_price_change(query_date, query_hour)
