...
def fetch_price_data(symbol, interval, start_time):
    ...
    try:
        ...
        logger.error(f"Failed to fetch data from proxy endpoint: {e}")
        logger.info("Attempting to fetch data from primary endpoint.")
...
def main():
    ...
    symbol = "ETHUSDT"
    interval = "1h"
    target_time_str = "2025-06-17 23:00:00"
    ...
    try:
        data = fetch_price_data(symbol, interval, start_time)
