...
def fetch_game_data(date):
    ...
    try:
        ...
        for game in games:
            if game['HomeTeam'] == 'WSH' and game['AwayTeam'] == 'NYI' or game['HomeTeam'] == 'NYI' and game['AwayTeam'] == 'WSH':
                logger.info(f"Found matching game: {game['AwayTeam']} at {game['HomeTeam']}")
                return game

        logger.warning(f"No matching game found between WSH and NYI on {date}.")
        return None
