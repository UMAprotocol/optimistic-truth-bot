...
def resolve_market():
    ...
    """
    symbol = "BTCUSDT"
    target_date = datetime(2025, 5, 30, 21, 0, 0, tzinfo=pytz.timezone("US/Eastern"))  # 9 PM ET
    ...
    try:
        ...
        # Compare with the opening price (1 hour earlier)
        opening_price = get_binance_price(symbol, start_time_ms - 3600000)
        logger.info(f"Opening price for {symbol} 1 hour earlier is {opening_price}")
