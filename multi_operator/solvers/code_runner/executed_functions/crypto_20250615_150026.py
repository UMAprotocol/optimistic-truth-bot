...
def resolve_market(data):
    """
    Resolves the market based on the change in price from the opening to the closing of the 1-hour candle.
    """
...
def main():
    """
    Main function to fetch the BTC/USDT price change for the 1-hour candle starting at June 15, 2025, 10 AM ET.
    """
    # Convert the target time to UTC timestamp
    tz = pytz.timezone("US/Eastern")
    target_time_local = tz.localize(datetime(2025, 6, 15, 10, 0, 0))
    target_time_utc = target_time_local.astimezone(timezone.utc)
    start_time_ms = int(target_time_utc.timestamp() * 1000)
    end_time_ms = start_time_ms + 3600000  # plus 1 hour

    # Fetch data
    data = get_data("BTCUSDT", "1h", start_time_ms, end_time_ms)
