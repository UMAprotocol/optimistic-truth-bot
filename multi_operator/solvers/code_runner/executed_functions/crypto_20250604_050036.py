...
def resolve_market():
    ...
    """
    symbol = "BTCUSDT"
    target_date = datetime(2025, 6, 4, 0, 0, 0, tzinfo=pytz.timezone("US/Eastern"))
    ...
    try:
        ...
        logger.info(f"Closing price at start of candle: {closing_price_start}")
        
        # Assuming we need to compare with the closing price at the end of the candle
        closing_price_end = get_binance_price(symbol, start_time_ms + 3600000)
        logger.info(f"Closing price at end of candle: {closing_price_end}")
