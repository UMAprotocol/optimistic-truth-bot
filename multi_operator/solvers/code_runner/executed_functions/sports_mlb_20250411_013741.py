...
RESOLUTION_MAP = {
    "Blackhawks": "p2",  # Blackhawks win maps to p2
    "Bruins": "p1",      # Bruins win maps to p1
...
def fetch_game_data():
    ...
    """
    date = "2025-04-10"
    home_team = "BOS"  # Boston Bruins
    away_team = "CHI"  # Chicago Blackhawks
...
def determine_resolution(game):
    ...
    if status == "Final":
        if home_score > away_score:
            logger.info("Home team (Bruins) won, resolving as Bruins")
            return RESOLUTION_MAP["Bruins"]
        elif away_score > home_score:
            logger.info("Away team (Blackhawks) won, resolving as Blackhawks")
            return RESOLUTION_MAP["Blackhawks"]
