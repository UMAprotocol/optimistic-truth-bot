...
def fetch_match_data():
    ...
    """
    # Placeholder URL and API key usage
    ...
    try:
        ...
        for match in matches:
            if match['HomeTeamName'] == "Athletic Club" and match['AwayTeamName'] == "Rangers":
                return match
            elif match['HomeTeamName'] == "Rangers" and match['AwayTeamName'] == "Athletic Club":
                return match

        logger.info("No match found for Athletic Club vs Rangers on the specified date.")
        return None
...
def determine_resolution(match):
    ...
    """
    if not match:
        logger.info("No match data available, resolving as 50-50 due to lack of information.")
        return RESOLUTION_MAP["50-50"]

    if match['Status'] == "Canceled" or match['Status'] == "Postponed":
        return RESOLUTION_MAP["50-50"]

    if match['Winner'] == "Home" and match['HomeTeamName'] == "Athletic Club":
        return RESOLUTION_MAP["ATH"]
    elif match['Winner'] == "Away" and match['AwayTeamName'] == "Athletic Club":
        return RESOLUTION_MAP["ATH"]
    elif match['Winner'] == "Home" and match['HomeTeamName'] == "Rangers":
        return RESOLUTION_MAP["RAN"]
    elif match['Winner'] == "Away" and match['AwayTeamName'] == "Rangers":
        return RESOLUTION_MAP["RAN"]
