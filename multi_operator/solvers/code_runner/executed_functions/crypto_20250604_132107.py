...
def fetch_data_from_binance(symbol, start_time, end_time):
    ...
    try:
        ...
        # If proxy fails, fall back to the primary endpoint
        try:
...
def check_solana_price_threshold(start_date, end_date, threshold_price):
    """
    Checks if the price of Solana ever reaches or exceeds the threshold price within the given date range.
    """
...
def main():
    # Define the date range for May 2025 in ET timezone
    start_date = "2025-05-01 00:00:00"
    end_date = "2025-05-31 23:59:59"
    threshold_price = 210.00

    # Convert ET to UTC
    et_timezone = pytz.timezone("US/Eastern")
    start_date_utc = et_timezone.localize(datetime.strptime(start_date, "%Y-%m-%d %H:%M:%S")).astimezone(pytz.utc)
    end_date_utc = et_timezone.localize(datetime.strptime(end_date, "%Y-%m-%d %H:%M:%S")).astimezone(pytz.utc)
    ...
    result = check_solana_price_threshold(start_date_str, end_date_str, threshold_price)

    # Print the result based on the outcome
    if result:
        print("recommendation: p2")  # Yes, Solana reached $210 or higher
    else:
        print("recommendation: p1")  # No, Solana did not reach $210
