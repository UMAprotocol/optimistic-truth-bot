...
def get_btc_price_at_specific_time(date_str, hour=5, minute=0, timezone_str="US/Eastern"):
    """
    Fetches the BTC/USDT 1-hour candle open and close prices on Binance at a specific time on a given date.
...
def main():
    ...
    """
    query_date = "2025-06-18"  # Date of the event
    logger.info(f"Processing price data for {query_date}")
    
    try:
        open_price, close_price = get_btc_price_at_specific_time(query_date)
