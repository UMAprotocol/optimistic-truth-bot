...
def resolve_market():
    """
    Resolves the market based on the price change of BTC/USDT on Binance for the specified candle.
    """
    # Define the specific date and time for the candle
    target_date = datetime(2025, 5, 29, 10, 0, 0, tzinfo=pytz.timezone('US/Eastern'))
    start_time = int(target_date.timestamp() * 1000)
    end_time = int((target_date + timedelta(hours=1)).timestamp() * 1000)

    # Fetch the candle data
    try:
        candle_data = get_data("BTCUSDT", start_time, end_time)
