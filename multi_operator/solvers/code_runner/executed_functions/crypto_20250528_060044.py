...
def convert_to_utc(year, month, day, hour, minute, timezone_str):
    """
    Converts local time to UTC time in milliseconds.
...
def main():
    ...
    # Convert the specified time to UTC in milliseconds
    ...
    try:
        ...
        print(f"The closing price of {symbol} at {datetime.utcfromtimestamp(start_time/1000).strftime('%Y-%m-%d %H:%M:%S')} UTC was {closing_price}")
    except Exception as e:
