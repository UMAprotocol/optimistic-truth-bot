...
RESOLUTION_MAP = {
    "Spurs": "p2",  # Spurs win maps to p2
    "Warriors": "p1",  # Warriors win maps to p1
...
def fetch_game_data():
    """
    Fetches game data for the specified NBA game between Spurs and Warriors.

    Returns:
        Game data dictionary or None if not found
    """
    date = "2025-04-09"
    home_team = "GSW"  # Golden State Warriors
    away_team = "SAS"  # San Antonio Spurs
...
def determine_resolution(game):
    ...
    logger.info(
        f"Game status: {status}, Score: Warriors {home_team_score} - Spurs {away_team_score}"
    )

    if status == "Final":
        if home_team_score > away_team_score:
            logger.info("Warriors won")
            return RESOLUTION_MAP["Warriors"]
        elif away_team_score > home_team_score:
            logger.info("Spurs won")
            return RESOLUTION_MAP["Spurs"]
    elif status == "Postponed":
        logger.info("Game postponed, too early to resolve")
        return RESOLUTION_MAP["Too early to resolve"]
    elif status == "Canceled":
        logger.info("Game canceled, resolving as 50-50")
