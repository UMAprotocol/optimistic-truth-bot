...
# Define paths
BASE_DIR = Path(__file__).parent.parent  # proposal_replayer directory
RESULTS_DIR = BASE_DIR / "results" / "14032025-gpt-refined-prompt" / "outputs"
OUTPUT_FILE = BASE_DIR / "combinatorial_follower" / "refined_prompt.txt"
...
def chat_completion(prompt):
    ...
    response = client.chat.completions.create(
        model="gpt-4-turbo", messages=messages, temperature=0.7
    )
...
def main():
    ...
    # Process files one by one, updating the prompt after each incorrect prediction
    for file_path in json_files:
        try:
            ...
            if recommendation != proposed_price_outcome:
                ...
                # Build the query for ChatGPT using the current refined prompt
                query = f"""
...
"""
                ...
                print(
                    f"\nProcessing incorrect prediction {incorrect_predictions}: {file_path.name}"
                )
                print(
                    f"Model recommended {recommendation}, but should have been {proposed_price_outcome}"
                )
                ...
                spinner_thread = threading.Thread(
                    ...
                    args=(
                        stop_event,
                        f"Refining prompt based on file {file_path.name}",
                    ),
                )
                ...
                try:
                    ...
                    if new_prompt:
                        # Update the refined prompt for the next iteration
                        refined_prompt = new_prompt
                        print(
                            f"\nPrompt updated successfully (iteration {incorrect_predictions})"
                        )
                finally:
                    stop_event.set()
                    spinner_thread.join()
    ...
    print(
        f"\n\nProcessed {incorrect_predictions} incorrect predictions out of {total_files} files"
    )
